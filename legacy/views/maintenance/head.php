...
<div class="flex items-center gap-3">
    ...
    <div x-data="{ open: false }" class="relative">
        ...
        <div x-show="open" 
            ...
            <?php if ($canClose and $id->ended_at and ! $id->closed_at) { ?>
                <li>
                    <a href="#" hx-get="?c=Maintenance&a=Update" hx-confirm='Are you sure?' hx-swap="none" hx-vals='{"id":<?= $id->id ?>,"field": "closed_at"}' class="block px-4 py-2 hover:bg-gray-100 flex items-center gap-2">
                ...
                <li>
                    ...
                    onclick="
                            ...
                            if (reason !== null && reason.trim() !== '') {
                                htmx.ajax('GET', '?c=Maintenance&a=Update&id=<?= $id->id ?>&field=reason&reason=' + encodeURIComponent(reason));
                            }
                    ">
                        <i class="ri-check-double-line"></i> Reject
                    </a>
                </li>
            <?php } ?>

            <?php if ($canEdit && ! $id->ended_at) { ?>
                <li>
                    <a href="#"
                    class="block px-4 py-2 hover:bg-gray-100 flex items-center gap-2"
                    hx-get="?c=Maintenance&a=Update"
                    hx-vals='{"id":<?= $id->id ?>, "field": "ended_at"}'
                    hx-confirm="Are you sure you wish to set this ticket as Attended?"
                    hx-swap="none"
                    hx-indicator="#loading">
                        <i class="ri-check-double-line"></i> Attend
                    </a>
                </li>
            <?php } ?>
            ...
            <?php if ($id->closed_at) { ?>
                <li>
                    ...
                    hx-get="?c=Maintenance&a=Modal&modal=rate&id=<?= $id->id ?>"
                    hx-target="#nestedModal"
                    hx-indicator="#loading">
                        <i class="ri-check-double-line"></i> Rate
                    </a>
                </li>
            <?php } ?>
            </ul>
