<div class="w-[95%] max-h-[98vh] sm:w-[30%] bg-white p-6 rounded-2xl shadow-lg relative z-50 overflow-y-auto">
  ...
  <form
    ...
    class="space-y-5"
    hx-post="?c=Maintenance&a=Save"
    hx-swap="none"
    hx-indicator="#loading"
  >

    <!-- Hidden type -->
    <input type="hidden" name="kind" value="Machinery">
    ...
    <div>
      ...
      >
        <option value=""></option>
        <?php foreach ($this->model->list('id,hostname,serial,sap', 'assets', "and (area = 'Machinery' or area = 'Vehicles') ORDER BY hostname ASC") as $r) { ?>
          <option value="<?= $r->id ?>"><?= mb_convert_case($r->hostname, MB_CASE_TITLE, 'UTF-8').' | '.$r->serial.' | '.$r->sap  ?></option>
        <?php } ?>
      </select>
    </div>
