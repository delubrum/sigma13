<div class="w-[95%] sm:w-[95%] bg-white p-4 rounded-lg shadow-lg relative z-50">
  ...
  <h1 class="text-black-700 mb-8"><i class="ri-add-line text-2xl"></i> <span class="text-2xl font-semibold"> Follow Plan <span></h1>
    <form 
    ...
    >
        ...
        <input type='hidden' name='plan_id' value="<?= $id->id ?>">
        <input type='hidden' name='infraimprovement_id' value="<?= $id->infraimprovement_id ?>">
        <input type='hidden' name='type' value="follow-plan">
    ...
    <input type="search" id="searchEvent" class="w-full px-3 py-2 border border-gray-300 rounded-md text-xs mb-3 shadow-inner focus:outline-none focus:ring-2 focus:ring-gray-500" 
        placeholder="Search..."
        name="search"
        hx-post="?c=Infraimprovement&a=GetEvents&kind=follow-plan&id=<?= $id->id ?>"
        hx-trigger="input changed delay:500ms, keyup[key=='Enter'], load"
        hx-target="#follow-results"
    ...
    <div id="follow-results"
        ...
        hx-get="?c=Infraimprovement&a=GetEvents&kind=follow-plan&id=<?= $id->id ?>"
