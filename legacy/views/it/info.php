<div class="p-4">
    ...
    <div class="mb-4 pb-2.5 border-b border-dashed border-gray-200 last:border-b-0 last:mb-0 last:pb-0">
        ...
        <?php
        $directorio = "uploads/serviceDesk/userpics/$id->id/";
            ...
            <?php if ($canClose) { ?>
                <div class="w-full">
                    <select 
                        ...
                        hx-post="?c=IT&a=Update"
                        hx-trigger="change"
                        hx-vals='{"id":<?= $id->id ?>,"field": "asset_id"}'
                    >
                        <option value=""></option>
                        <?php
                    foreach ($this->model->list('id,hostname,serial,sap', 'assets', " AND area = 'IT' ORDER BY serial ASC") as $r) {
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="mb-4 pb-2.5 border-b border-dashed border-gray-200 md:border-b-0">
            ...
            <div class="flex text-xs mb-1">
                <div class="w-full">
                    <select required name="sgc" class="w-full p-2 border border-gray-300 rounded-md" hx-post="?c=IT&a=Update" hx-trigger="change" hx-vals='{"id":<?= $id->id ?>,"field": "sgc"}'>
                        <option value=""></option>
                        <?php
...
<div class="mb-4 pb-2.5 border-b border-dashed border-gray-200 last:border-b-0 last:mb-0 last:pb-0">
    ...
    <div class="flex text-xs mb-1">
        <?php if ($canClose) { ?>
            <div class="w-full">
                ...
                >
                    <option value=""></option>
                    <?php
                    // FIX: Cambiamos JSON_CONTAINS por el operador @> de Postgres
                    // La sintaxis '["30"]' indica que buscamos ese valor dentro del array JSONB
                    $tecnicos = $this->model->list('id,username', 'users', " AND active = true AND permissions::jsonb @> '[\"30\"]'::jsonb ORDER BY username ASC");

            foreach ($tecnicos as $r) {
        ...
        <?php } elseif ($id->assignee_id != null) { ?>
            <div class="font-medium text-gray-900">
                <?= mb_convert_case($id->assignee_name ?? $id->assignee, MB_CASE_TITLE, 'UTF-8') ?>
            </div>
        <?php } ?>
    </div>
</div>
