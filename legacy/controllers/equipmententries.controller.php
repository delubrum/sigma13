...
class EquipmentEntriesController
{
    public function __construct(public Model $model, private AuthService $auth) {}

    public function Index()
    {
        $user = $this->auth->authorize(156);
        $tabulator = true;
        $jspreadsheet = true;
        $title = 'OHS / Equipment / Entries';
        $button = 'New Entry';
    ...
    public function Data()
    {
        ...
        $totalQuery = $this->model->get(
            ...
            'equipment_register a',
            "$where",
            'LEFT JOIN equipment_db b ON a.item_id = b.id'
        );
        ...
        $data = [];
        foreach ($rows as $r) {
            $out = $this->model->get('count(name) as total', 'equipment', " AND name = '$r->name'")->total;
    ...
    {
        $user = $this->auth->authorize(156);
        require_once 'app/views/equipment/new-entry.php';
    }
    ...
    public function Save()
    {
        ...
        $id = $this->model->save('equipment_register', $item);
    ...
    {
        $user = $this->auth->authorize(156);
        require_once 'app/views/equipment/detail.php';
    }
    ...
    public function DetailData()
    {
        ...
        SALIDAS (Out)
        ======================*/
        $itemInfo = $this->model->get('name', 'equipment_db', " AND id = $id");
