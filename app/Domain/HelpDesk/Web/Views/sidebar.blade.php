<div class="p-4 space-y-4">
    ...
    {{-- Asset --}}
    <x-sidebar-section icon="ri-tools-line" label="Activo">
        @if($data->canEdit && count($data->assets) > 0)
            <select name="asset_id" data-widget="tomselect"
    ...
    <div class="grid grid-cols-2 gap-3">
        <x-sidebar-section icon="ri-flag-line" label="Prioridad">
            @if($data->canEdit)
                <select name="priority" data-widget="tomselect"
                        hx-post="{{ route('global.patch', ['helpdesk', $data->id]) }}"
                        hx-trigger="change delay:50ms"
        ...
        <x-sidebar-section icon="ri-user-star-line" label="Asignación">
            @if($data->canEdit && count($data->technicians) > 0)
                <select name="assignee_id" data-widget="tomselect"
                        hx-post="{{ route('global.patch', ['helpdesk', $data->id]) }}"
                        hx-trigger="change delay:50ms"
    ...
    <div class="grid grid-cols-2 gap-3">
        <x-sidebar-section icon="ri-settings-3-line" label="SGC">
            <select name="sgc_code" data-widget="tomselect"
                    hx-post="{{ route('global.patch', ['helpdesk', $data->id]) }}"
                    hx-trigger="change delay:50ms"
        ...
        <x-sidebar-section icon="ri-error-warning-line" label="Causa">
            <select name="root_cause" data-widget="tomselect"
                    hx-post="{{ route('global.patch', ['helpdesk', $data->id]) }}"
                    hx-trigger="change delay:50ms"
