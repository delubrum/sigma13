...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9f9f9;">
    <div style="background-color: white; border-radius: 12px; padding: 40px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
        
        @if($icon)
            <div style="margin-bottom: 24px;">
                <span style="font-size: 40px; color: {{ $color ?? '#171717' }};">
                     {!! str_starts_with($icon, 'ri-') ? '🔑' : $icon !!} 
                </span>
            </div>
        @endif
