...
@php
    $inputId = 'field-' . $field->name . '-' . (data_get($data, 'id') ?? Str::random(4));
    $value = $value ?? (data_get($data, $field->name) ?? '');

    if ($value instanceof \DateTimeInterface) {
        $value = $value->format($field->type === 'date' ? 'Y-m-d' : 'Y-m-d H:i:s');
    }

    $baseClasses = "w-full px-3 py-2 rounded-lg text-sm font-semibold outline-none transition-all border";
    ...
    // Condicional: este campo depende de otro
    $hasCondition = $field->dependsOn && $field->showWhen !== null;
    $showWhenJs   = json_encode((array) $field->showWhen);
@endphp
...
@else
    ...
    <div class="flex flex-col gap-1 {{ $class }}"
         x-show="{{ $showWhenJs }}.map(String).includes(String(_deps['{{ $field->dependsOn }}'] ?? ''))"
         x-cloak>
    @else
    <div class="flex flex-col gap-1 {{ $class }}">
    @endif
        ...
        @if ($hasLabel)
            <label for="{{ $inputId }}" class="text-xs font-black uppercase tracking-widest" style="color:var(--tx2)">
                {{ $field->label }}
        ...
        @elseif ($field->widget === 'slimselect' || $field->widget === 'tomselect')
            @if($hasCondition && is_array($field->options) && count($field->options) > 0 && is_array(reset($field->options)))
                {{-- Opciones agrupadas por área: [{group, value, label}] --}}
                <select id="{{ $inputId }}" name="{{ $field->name }}"
                        {{ $field->required ? 'required' : '' }}
                        x-cloak
                        data-widget="tomselect"
                        data-depends-on="{{ $field->dependsOn }}"
                        data-show-when="{{ json_encode((array) $field->showWhen) }}"
                        data-all-options="{{ json_encode($field->options) }}"
                        {!! $hxAttributes !!}>
                    <option value="">— {{ $field->placeholder ?: 'Seleccionar' }} —</option>
                </select>
            @else
                <select id="{{ $inputId }}" name="{{ $field->name }}" data-widget="tomselect"
                        {{ $field->required ? 'required' : '' }}
                        @if($field->showWhen !== null) x-cloak @endif
                        @change="_track('{{ $field->name }}', $event.target.value)"
                        {!! $hxAttributes !!}>
                    <option value="">— {{ $field->placeholder ?: 'Seleccionar' }} —</option>
        ...
        @elseif ($field->type === 'select')
            <select id="{{ $inputId }}" name="{{ $field->name }}"
                    {{ $field->required ? 'required' : '' }}
                    @change="_track('{{ $field->name }}', $event.target.value)"
                    class="{{ $baseClasses }}" style="{{ $style }}" {!! $hxAttributes !!}>
                <option value="">— {{ $field->placeholder ?: 'Seleccionar' }} —</option>
                @foreach ($field->options as $optValue => $optLabel)
                    <option value="{{ $optValue }}" @if($value == $optValue) selected @endif>{{ $optLabel }}</option>
                @endforeach
        ...
        @elseif ($field->type === 'textarea')
            <textarea id="{{ $inputId }}" name="{{ $field->name }}" rows="{{ $field->rows ?? 3 }}" placeholder="{{ $field->placeholder }}" {{ $field->required ? 'required' : '' }} class="{{ $baseClasses }} resize-none" style="{{ $style }}" {!! $hxAttributes !!}>{{ $value }}</textarea>
        ...
        @if ($field->hint)
            <p class="text-xs" style="color:var(--tx2); opacity:.5">{{ $field->hint }}</p>
        @endif
    </div>
@endif
