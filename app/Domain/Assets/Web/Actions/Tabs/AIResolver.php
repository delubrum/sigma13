...
final class AIResolver
{
    ...
    {
        return new Config(
            title: $asset ? "Análisis AI: <span class='opacity-50'>{$asset->serial}</span>" : 'SIGMA AI Analysis',
    ...
    {
        return $this->hxView('assets::tabs.ai', [
            'asset' => $asset,
    ...
    public function asGenerate(Request $request, Asset $asset): Response
    {
        ...
        try {
            ...
            if (count($results) === 0) {
                return response('<div class="p-8 text-center bg-rose-500/10 border border-border rounded-2xl font-black text-rose-600 uppercase">Sin históricos.</div>');
            }
...
";
            ...
            usort($freq, fn (array $a, array $b): int => $b['pct'] <=> $a['pct']);
            $analysis['failures_by_frequency'] = $freq;

            $impact = $analysis['failures_by_impact'];
            usort($impact, fn (array $a, array $b): int => $b['pct'] <=> $a['pct']);
            $analysis['failures_by_impact'] = $impact;

            return $this->hxFragment('assets.tabs.ai', 'report', [
                'asset' => $asset,
                'metrics' => $metrics,
            ...
            return response('<div class="p-4 bg-red-600 text-white font-black text-xs">Engine Error: '.$e->getMessage().'</div>');
        }
    }
}
