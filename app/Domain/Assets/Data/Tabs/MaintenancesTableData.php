...
final class MaintenancesTableData extends Data
{
    ...
    {
        $date = isset($row->created_at) && $row->created_at !== null
            ? Carbon::parse((string) $row->created_at)->format('d/m/Y H:i')
            : '---';

        $closed = isset($row->ended_at) && $row->ended_at !== null
            ? Carbon::parse((string) $row->ended_at)->format('d/m/Y H:i')
            : '---';

        return new self(
            id:          (int) $row->id,
            type:        $row->subtype ?? 'Corrective',
            date:        $date,
            user:        $row->user_name ?? '---',
