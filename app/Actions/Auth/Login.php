...
final class Login
{
    ...
    public function handle(Form $data): bool
    {
        return Auth::attempt(
    ...
    private function checkIfBlocked(string $key): ?JsonResponse
    {
        ...
        $minutes = (int) config('auth.login_decay_minutes', 15);
    ...
    private function verifyRecaptcha(?string $token, string $ip): ?JsonResponse
    {
        ...
        $result = new ReCaptcha(config('services.recaptcha.secret'))
            ->setExpectedHostname(parse_url((string) config('app.url'), PHP_URL_HOST))
            ->verify($token, $ip);
    ...
    {
        $max = (int) config('auth.login_max_attempts', 5);
        $decay = (int) config('auth.login_decay_minutes', 15);
